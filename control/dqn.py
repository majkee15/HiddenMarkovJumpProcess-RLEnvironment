...
from control.memory import Transition, ReplayMemory, PrioritizedReplayMemory
from control.base import Policy, BaseModelMixin, TrainConfigBase
from control.utils.misc import plot_learning_curve, plot_to_image
from control.utils.construct_nn import construct_nn
from control.utils.annealing_schedule import LinearSchedule
...
class DQNAgent(BaseModelMixin, Policy):
    ...
    def __init__(self, env, name, config=None, training=True,  initialize=False, experiment_name=None):
    ...
    def run_training(self):
        ...
        for i in range(n_episodes):
            ...
            if i % self.config.log_every_episode == 0:
                ...
                self.logger.warning(f"episode:{i}/{self.config.n_episodes} episode reward: {score} avg reward (last 100): "
...
if __name__ == '__main__':
    ...
    env = HMMJPEnvSustAct(params=params, t_terminal=10, track_intensity=True, reward_shaping='continuous')
    ...
    dqn = DQNAgent(env, 'HMM_test', training=True, config=DefaultConfig(), initialize=False, experiment_name=experiment_name)
    dqn.run_training()